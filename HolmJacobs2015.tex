...
\end{align*}
where $\varphi^* \omega$ denotes the distribution defined by
\begin{align*}
	\int f(z) (\varphi^*\omega)(z) := \int f(\varphi(z)) \omega(z) \quad , \quad \forall f \in C^\infty(\R^2).
\end{align*}
...
The symplectic structure is nothing more than a special case of one derived via the KKS theorem \cite[see the boxed formula on p.303]{FOM}.
