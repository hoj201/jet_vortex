...
\begin{align}
 \partial_t \omega + u \partial_x \omega + v \partial_y \omega = 0. \label{eq:vorticity}
\end{align}
...
where $\Gamma_i \in \mathbb{R}$, $z=(x,y) \in \mathbb{R}^2$ and $\delta_{z_i}(z) :=\delta(z-z_i)$ is the Dirac delta distribution centered at the point $z_i = (x_i,y_i) \in \mathbb{R}^2$.
...
The numerical aspects extended the standard vortex blob method considerably.
For example, we observed faster convergence rates in space using higher order jet vortices.
We also proposed a scheme to decrease the number of pairwise interactions, by grouping
