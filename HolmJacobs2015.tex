...
In our case $S = {\rm Orb}(\omega_0) = \{ \omega_0 \circ \varphi^{-1} \mid \varphi \in \SDiff(\mathbb{R}^2) \}$ is a coadjoint orbit of some vorticity distribution on $\mathbb{R}^2$, and the sympectic form at a ``point'' $\omega \in S$ is given by Kostant's formula
\begin{align*}
  \Omega_\omega( \pounds_u[\omega] , \pounds_v [\omega] ) :=
  \langle \omega , u \times v \rangle.
\end{align*}
where $u,v \in \mathfrak{X}(\mathbb{R}^2)$ and $u \times v \in C^{\infty}(\mathbb{R}^2)$ (see \S \ref{sec:symplectic} of this article).
...
This fact induces an action on $S$.
In particular, for any $(a,b) \in \mathbb{R}^2$, there is a natural action on $C^{\infty}(\mathbb{R}^2)$ by pull-back, sending the function $\phi(z)$ to the function  $(a,b)_*\phi(z) := \phi(z+(a,b))$.  The corresponding action on vector-fields and all other objects on $\mathbb{R}^2$ follows naturally.  
 In particular, the left-action on distributions sends $\omega \in \mathcal{D}(\R^2)$ to the distribution $(a,b)_*\omega$ given by
\begin{align*}
  \langle (a,b)_* \omega , \phi \rangle := \langle \omega , (a,b)^* \phi \rangle.
\end{align*}
As translation of $\mathbb{R}^2$ by $(a,b)$ is a volume-preserving diffeomorphism, we see that $S \subset \mathcal{D}(\mathbb{R}^2)$ is invariant under this action.
Moreover, we observe the action on $S$ is symplectic because
\begin{align*}
...
In this section we consider the action of rotation about the origin in $\R^2$.
This action on $\R^2$ will induce a symplectic Lie group action on the symplectic manifold $S = {\rm Orb}(\omega_0)$ derived in \S \ref{sec:symplectic}.
