...
\subsection{The {vortex blob} method}
The concept of regularising the singular vortex solutions of EulerÍs equations for the incompressible flow of an ideal fluid goes back to Rosenhead \cite{Ro1931} who refers to Kelvin.
The introduction to Chorin's 1993 Les Houches lecture notes \cite{Ch1993} provides a quick synopsis of the development and application of vortex particle methods for incompressible flows since then.
%
\begin{quote}
We shall restrict ourselves in these lectures to a special class of numerical vortex
methods, those that are based on a Lagrangian transport of vorticity in hydrodynamics by
smoothed particles (``blobs") and those whose analysis contributes to the understanding of
blob methods. Blob methods started in the thirties as two-dimensional ``point" methods
\cite{Ro1931}. By the fifties, it was discovered that ``point vortex" methods had drawbacks, and
a misinterpretation of the Poincar\'e recurrence theorem led to the conclusion that the
drawbacks could not be remedied (for an analysis, see \cite{Kr1986}). In the late sixties and
early seventies, the virtues of smoothing were discovered \cite{Ch1972,Ch1973,ChBe1973,Ch1993} and viscosity and boundaries were added.
The generalization to three dimensions followed soon \cite{Ch1980,Le1975,Le1985} and was found to
be non-unique. Arrows, filaments, dipoles, magnets, all generalize two-dimensional blobs, \dots
\end{quote}
%
For more details and subsequent developments, we refer to the vast literature on this subject, partially summarised with over 400 references in \cite{Stock2007}. \\

Our motivation in the present paper is to use several recent developments in the geometric properties of  incompressible fluid mechanics \cite{CotterHolmJacobsMeier2014} to make the extension of vortex blob dynamics to include higher order derivatives in the singular representation of the vorticity. We aim to investigate the evolution of the shapes of those blobs and the dynamics of their mutual interactions. These higher order singular vortices will be called jet vortices, because they correspond to the jets (Taylor expansion coefficients) of the vortex blob Greens functions.
