...
\todo[inline]{I have not found a way to avoid writing Lie-derivatives.
  Originally, I wanted to write $u^i \partial_i \omega$ in place of $\pounds_{u}[\omega]$.
  However, these are not equivalent when $\omega$ is a generic distribution.  Consider $\omega = \partial_x \delta_z$.  Then 
  \begin{align*}
    \langle \pounds_u [\omega] , f \rangle &= - \langle \omega , u^i \partial_i f \rangle \\
    &= \partial_x |_{z} ( u^i \partial_i f) \\
    &= \partial_x u^i(z) \partial_i f(z) + u^i(z) \partial_{xi} f(z) \\
    &= \langle -\partial_x u^i(z) \partial_i \delta_z + u^i(z) \partial_{xi}\delta_z , f \rangle
  \end{align*}
  Thus $\pounds_u[\omega] = -\partial_x u^i(z) \partial_i \delta_z + u^i(z) \partial_{xi}\delta_z$.
  This differs from $u^i \partial_{ix} \delta_z$.}
...
\todo[inline]{There is still more the think about here.  We might have more conservation laws.}
